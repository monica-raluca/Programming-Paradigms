...
-- fromPoints points = \p -> any (\q -> p == q) points
fromPoints = flip (any . (==))
...
rectangle width height = \(x, y) -> (x <= halfWidth && x >= -halfWidth) && (y <= halfHeight && y >= -halfHeight)
  where
    halfWidth = width / 2
    halfHeight = height / 2
...
-- promoteUnary f pointed = f . pointed
-- promoteUnary = fmap
promoteUnary = (.)
...
-- applyTransformation = \transform reg -> reg . transform
...
-- bfs start expand = snd $ mapAccumL explore (S.singleton start, 0) [(start, 0)] 
--   where
--     -- explore :: (S.Set a, Int) -> (a, Int) -> ((S.Set a, Int), (a, Int))
--     explore (visited, dist) (state, currDist) =
--       let neighbors = filter (\(n, _) -> S.notMember n visited) (map (\n -> (n, currDist + 1)) (expand state))
--           newVisited = foldl (\acc (x, _) -> S.insert x acc) visited neighbors
--           newDist = currDist + 1
--           newState = map (\(n, _) -> (n, newDist)) neighbors
    --   in ((newVisited, newDist), (newState, currDist))

-- bfs :: (Ord a) => a -> (a -> [a]) -> [(a, Int)]
-- bfs start expand = snd result
--   where
--     result = mapAccumL bfsStep (S.singleton start, 0) [(start, 0)]

--     bfsStep (visited, depth) (state, currDist) =
--         let successors = filter (`S.notMember` visited) (expand state)
--             newVisited = foldr S.insert visited successors
--         in ((newVisited, depth + 1), (newVisited, depth))

-- bfs :: (Ord a) => a -> (a -> [a]) -> [(a, Int)]
-- bfs start expand = snd result
--   where
--     result = mapAccumL bfsStep (S.singleton start, 0) $ start : concatMap expand (fst result)

-- bfsStep (visited, depth) state =
--       let successors = filter (`S.notMember` visited) (expand state)
--           newVisited = foldr S.insert visited successors
--       in ((newVisited, depth), (successors, depth + 1))

bfs :: (Ord a) => a -> (a -> [a]) -> [(a, Int)]
-- initial list has a set with the start node and a list with the start node and distance 0
bfs start expand = unfoldr bfsStep (S.singleton start, [(start, 0)])
  where
    bfsStep (visited, []) = Nothing -- we stop when we have no more nodes
    -- we extract our current node and distance, and we are left with the rest of the nodes
    bfsStep (visited, (currentNode, dist) : rest) =
      let neighbors = filter (`S.notMember` visited) (expand currentNode) -- we filter out nodes we have already visited
          newVisited = foldr S.insert visited neighbors -- we add the new neighbors
          newNodes = rest ++ [(n, dist + 1) | n <- neighbors]
      in Just ((currentNode, dist), (newVisited, newNodes))
...
regionAvoidingBfs start region = bfs start (\p -> filter (not . region) (expand p)) -- filter out points that are in our region
  where
