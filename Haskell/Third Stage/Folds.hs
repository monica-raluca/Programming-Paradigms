...
instance Show RegionAST where
  show (R region) = show' (R region) ""
    where
      show' (R region) spaces = let
          indentedShow = (spaces ++)
          nestedShow = (`show'` (spaces ++ replicate 2 ' '))
        in case region of
          (Complement region) -> indentedShow "~\n" ++ nestedShow region
          (Union region1 region2) -> indentedShow "+\n" ++ nestedShow region1 ++ "\n" ++ nestedShow region2
          (Intersection region1 region2) -> indentedShow "*\n" ++ nestedShow region1 ++ "\n" ++ nestedShow region2
          (Transform transformation region) -> indentedShow (show transformation ++ "\n") ++ nestedShow region
          _ -> indentedShow (show region)
...
basicEntityCount = foldRegionAST combiner
  where
    ...
    combiner region =
      case region of
        Complement (r1, t1) -> (r1, t1)
        Union (r1, t1) (r2, t2) -> (r1 + r2, t1 + t2)
        Intersection (r1, t1) (r2, t2) -> (r1 + r2, t1 + t2)
        Transform transformation (r, t) -> (r, t + basicTransformationCount transformation)
        _ -> (1, 0)
