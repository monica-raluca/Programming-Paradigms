...
(define (get-suffixes text)
  (define (get-suffixes-rec text result)
    (cond
      ((null? text) result)
      (else (cons text (get-suffixes-rec (cdr text) result)))
      ))
...
(define (get-ch-words words ch)
  (filter (lambda (x) (equal? ch (car x))) words))
...
(define (ast-func suffixes)
  (cons (take (car suffixes) 1) (map (lambda (x) (list-tail x 1)) suffixes)))
...
(define (cst-func suffixes)
  (cons (longest-common-prefix-of-list suffixes) (map (lambda (x) (list-tail x (length (longest-common-prefix-of-list suffixes)))) suffixes)))
...
(define (suffixes->st labeling-func suffixes alphabet)
  (let* ((suffix-branches (filter (lambda (x) (not (null? x)))(map (lambda (x) (get-ch-words suffixes x)) alphabet)))
         (partial-trees (map (lambda (x) (labeling-func x)) suffix-branches))
         (no-null (map (lambda (x) (filter (lambda (x) (not (null? x))) x)) partial-trees)))
    (map (lambda (x) (if (null? (cdr x)) (list(car x)) (cons (car x) (suffixes->st labeling-func (cdr x) alphabet)))) no-null)
    ))
