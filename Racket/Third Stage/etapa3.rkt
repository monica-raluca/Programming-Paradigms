...
(define (longest-common-substring text1 text2)
  (let* ((st1 (text->cst text1))
    ...
    (let loop ((suffixes suffixes) (result '()))
      (if (null? suffixes)
          result
          (let find-match ((st1 st1) (label (car suffixes)) (partial-result '()))
            (let* ((temp (match-pattern-with-label st1 label))
                   (new-result (append partial-result label))
                   (rest-suffixes (cdr suffixes)))
              (cond
                ((boolean? temp) (if (> (length new-result) (length result))
                                     (loop rest-suffixes new-result)
                                     (loop rest-suffixes result)))
                ((not (car temp)) (if (> (length (append partial-result (cadr temp))) (length result))
                                      (loop rest-suffixes (append partial-result (cadr temp)))
                                      (loop rest-suffixes result)))
                (else (find-match (caddr temp) (cadr temp) (append partial-result (car temp))))
                )
              )
            )
          )
